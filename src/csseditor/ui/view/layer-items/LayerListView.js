...
export default class LayerListView extends UIElement {
                ...
                <div class="layer-list-toolbar">
                    ...
                    <span class='layer-tools'>
                        <div class="button-group">
                            <button type="button" ref="$addArtBoard" title="add ArtBoard">${
                              icon.add_note
                            }</button>
                            <button type="button" ref="$addDirectory" title="add Directory">${
                              icon.create_folder
                            }</button>
                        </div>
                    </span> 
  ...
  makeItem(item, depth) {
    ...
    return html`
      <div
        class="tree-item depth-${depth} ${selected} ${item.index}"
        item-id="${item.id}"
      ...
      >
        ...
        <div class="item-title" data-label="${label}">
          ${item.toGridString()} ${item.title}
        </div>
        <div class="item-tools">
          ${hasLock &&
            `<button type="button" class='lock-item ${lock}' title="Visible">${
              icon.lock
            }</button>`}
          ${hasVisible &&
            `<button type="button" class='visible-item ${visible}' title="Visible">${
              icon.visible
            }</button>`}
  ...
  [EVENT("refreshLayerListView")]() {
    this.refresh();
  }

  // all effect
  [EVENT(CHANGE_EDITOR, COPY_ITEMS)]() {
    this.refresh();
  }
  ...
  [EVENT(CHANGE_LAYER, CHANGE_RECT, CHANGE_ARTBOARD)]() {
    this.refreshLayer();
  }
  ...
  [CLICK("$addArtBoard")]() {
    var project = editor.selection.currentProject;
    if (project) {
      var artboard = project.addArtBoard(
        new ArtBoard({
          name: "New ArtBoard"
        })
      );
  ...
  [CLICK("$addDirectory")]() {
    ...
    if (currentItem) {
      if (currentItem instanceof ArtBoard || currentItem instanceof Directory) {
        var directory = currentItem.add(
          new Directory({
            name: "New Directory"
          })
        );
      } else if (currentItem instanceof Layer) {
        var directory = currentItem.parentDirectory().add(
          new Directory({
            name: "New Directory",
