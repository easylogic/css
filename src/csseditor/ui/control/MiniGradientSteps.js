...
export default class MiniGradientSteps extends UIElement {
  initialize() {
    super.initialize();
  ...
  getUnitSelect(step) {
    return `
        <select class='unit' data-colorstep-id="${step.id}">
            ...
            <option value='px' ${
              step.isPx ? "selected" : EMPTY_STRING
            }>px</option>
            <option value='em' ${
              step.isEm ? "selected" : EMPTY_STRING
            }>em</option>
        </select>
        `;
  }
  ...
  [LOAD("$stepList")]() {
    return this.colorsteps.map(step => {
      var cut = step.cut ? "cut" : EMPTY_STRING;
      return `
                <div 
                    class='drag-bar ${
                      step.selected ? "selected" : EMPTY_STRING
                    }' 
                    id="${step.id}"
                    style="left: ${this.getStepPosition(step)}px;"
                >   
                    <div class="guide-step step" style=" border-color: ${
                      step.color
                    };background-color: ${step.color};"></div>
                    <div class='guide-line' 
                        style="background-image: linear-gradient(to bottom, rgba(0, 0, 0, 0), ${
                          step.color
                        } 10%) ;"></div>
                    <div class="guide-change ${cut}" data-colorstep-id="${
        step.id
      }"></div>
                </div>
            `;
  ...
  setBackgroundColor() {
    this.refs.$stepList.css(
      "background-image",
      LinearGradient.toLinearGradient(this.colorsteps)
    );
  }
  ...
  [EVENT(CHANGE_COLORSTEP, CHANGE_EDITOR, CHANGE_SELECTION)]() {
    this.refresh();
  }
